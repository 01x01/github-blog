...
<!-- more -->
# basic
XSS 是 web 中最常见也是最基本入门的漏洞，分为三种类型：
1. 反射型 xss： 用户输入--> 服务器接收用户输入 --> 将用户输入返回到源代码 html 中
2. DOM 型 xss：用户输入 --> 直接返回到到 html 中,
3. 存储型 xss：用户输入 --> 存储到服务器数据库中 --> 其他用户访问 --> 接受到数据库中的 xss

DOM XSS 跟正常的反射型 XSS区别在于：
